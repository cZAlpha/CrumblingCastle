...
# Movement Variables
const ACCELERATION = 500 # The acceleration for the slime's movement
const MAX_SPEED = 40 # The maximum speed the slime can reach
const THRESHOLD_DISTANCE = 200 # Distance threshold for slime to start moving

# State Variables
enum {IDLE, ATTACK, DEAD} # All possible states
...
func _ready():
	...
	SoundManager.playSoundFX(id, load("res://Assets/Sound/SFX/Slime/Idle/slime-2-30099.mp3"))
...
func remove_slime():
	GameManager.finish_quest("Kill a Slime!") # Finishes the kill a slime quest if its active
